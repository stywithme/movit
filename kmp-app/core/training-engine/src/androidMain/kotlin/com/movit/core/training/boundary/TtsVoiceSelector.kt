...
/**
 * Selects the optimal TTS voice for a given language, preferring male voices and higher-quality
 * neural engines when available (ported verbatim from the legacy training engine).
 *
...
internal object TtsVoiceSelector {
     ...
     *
     * Guarantee: always calls setLanguage() or setVoice() — TTS will produce sound even on
     * devices with no ideal voice.
     *
    ...
    private fun pickAndApply(
        ...
        Log.d(
            TAG,
            "Selected voice: name=${best.name}, locale=${best.locale}, " +
    ...
    private fun scoreVoice(voice: Voice, language: String): Int {
        ...
        if (language == "ar") {
            ...
            if (voice.locale == Locale.US) {
                score += 100
            } else if (voice.locale.language == "en") {
                score += 50
            }
        }
    ...
    /**
     * Heuristic to guess male gender from voice metadata (Android API does not expose gender).
     * Falls back to false (unknown) — never penalises, just doesn't get the bonus.
     */
