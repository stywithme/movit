...
object CameraPositionDetector {
    ...
    fun detect(landmarks: List<Landmark>): CameraDetectionResult {
    ...
    internal fun computeMetrics(landmarks: List<Landmark>): DetectionMetrics? {
        if (landmarks.size < 33) return null

        val lShoulder = landmarks[PoseLandmarkIndices.LEFT_SHOULDER]
        val rShoulder = landmarks[PoseLandmarkIndices.RIGHT_SHOULDER]
        val lHip = landmarks[PoseLandmarkIndices.LEFT_HIP]
        val rHip = landmarks[PoseLandmarkIndices.RIGHT_HIP]
        val lKnee = landmarks[PoseLandmarkIndices.LEFT_KNEE]
        val rKnee = landmarks[PoseLandmarkIndices.RIGHT_KNEE]
    ...
    fun computeFaceVisibilityScore(landmarks: List<Landmark>): Float {
        val nose = landmarks.getOrNull(PoseLandmarkIndices.NOSE)?.visibility ?: 0f
        val leftEye = landmarks.getOrNull(PoseLandmarkIndices.LEFT_EYE)?.visibility ?: 0f
        val rightEye = landmarks.getOrNull(PoseLandmarkIndices.RIGHT_EYE)?.visibility ?: 0f
        val leftEar = landmarks.getOrNull(PoseLandmarkIndices.LEFT_EAR)?.visibility ?: 0f
        val rightEar = landmarks.getOrNull(PoseLandmarkIndices.RIGHT_EAR)?.visibility ?: 0f
    ...
    internal fun computeDepthInfo(landmarks: List<Landmark>): DepthInfo {
        val lS = landmarks[PoseLandmarkIndices.LEFT_SHOULDER]
        val rS = landmarks[PoseLandmarkIndices.RIGHT_SHOULDER]
        val lH = landmarks[PoseLandmarkIndices.LEFT_HIP]
        val rH = landmarks[PoseLandmarkIndices.RIGHT_HIP]
        val lK = landmarks[PoseLandmarkIndices.LEFT_KNEE]
        val rK = landmarks[PoseLandmarkIndices.RIGHT_KNEE]
        ...
        val indices = intArrayOf(
            ...
            PoseLandmarkIndices.LEFT_KNEE, PoseLandmarkIndices.RIGHT_KNEE
        )
...
class StableCameraDetector(
    ...
    fun detect(landmarks: List<Landmark>, isFrontCamera: Boolean = false): CameraPositionDetector.CameraDetectionResult {
