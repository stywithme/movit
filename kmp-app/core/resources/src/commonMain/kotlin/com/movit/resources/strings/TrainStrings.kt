...
data class TrainStrings(
    ...
    companion object {
        // Split load avoids JVM MethodTooLargeException on Android resource compile.
        suspend fun load(language: String): TrainStrings {
            val core = loadCore(language)
            val copy = loadCopy(language)
            val status = loadStatus(language)
            return TrainStrings(
                language = language,
                title = core.title,
                activeProgram = core.activeProgram,
                activePlan = core.activePlan,
                noActiveProgram = core.noActiveProgram,
                zeroExercises = core.zeroExercises,
                explorePrograms = core.explorePrograms,
                programComplete = core.programComplete,
                programCompleteSub = core.programCompleteSub,
                viewReport = core.viewReport,
                activeRecovery = core.activeRecovery,
                recoveryDay = core.recoveryDay,
                recoverToday = core.recoverToday,
                noWorkout = core.noWorkout,
                recovery = core.recovery,
                exploreRecovery = core.exploreRecovery,
                training = core.training,
                startSession = core.startSession,
                completed = core.completed,
                ready = core.ready,
                todaysWorkout = core.todaysWorkout,
                viewSummary = core.viewSummary,
                thisWeek = core.thisWeek,
                readiness = core.readiness,
                latestReport = core.latestReport,
                lastWorkout = core.lastWorkout,
                metricForm = core.metricForm,
                metricReps = core.metricReps,
                metricStreak = core.metricStreak,
                metricMinutes = core.metricMinutes,
                programFallback = core.programFallback,
                featured = core.featured,
                workComplete = core.workComplete,
                currentDay = core.currentDay,
                statusRecovery = core.statusRecovery,
                findPlan = core.findPlan,
                assessment = core.assessment,
                reassessmentDue = core.reassessmentDue,
                startBodyScan = core.startBodyScan,
                startReassessment = core.startReassessment,
                subtitleActive = copy.subtitleActive,
                subtitleCompleteToday = copy.subtitleCompleteToday,
                subtitleRest = copy.subtitleRest,
                subtitleProgramComplete = copy.subtitleProgramComplete,
                subtitleNoAssessment = copy.subtitleNoAssessment,
                subtitleReassessment = copy.subtitleReassessment,
                subtitleNoPlan = copy.subtitleNoPlan,
                readyMsgActive = copy.readyMsgActive,
                readyMsgRest = copy.readyMsgRest,
                readyMsgComplete = copy.readyMsgComplete,
                readyMsgProgramDone = copy.readyMsgProgramDone,
                readyMsgNoPlan = copy.readyMsgNoPlan,
                guideActive = copy.guideActive,
                guideRest = copy.guideRest,
                guideComplete = copy.guideComplete,
                guideProgramDone = copy.guideProgramDone,
                guideNoPlan = copy.guideNoPlan,
                quickExplore = copy.quickExplore,
                quickExploreSub = copy.quickExploreSub,
                quickReports = copy.quickReports,
                quickReportsSub = copy.quickReportsSub,
                quickTune = copy.quickTune,
                quickTuneSub = copy.quickTuneSub,
                prefsLater = copy.prefsLater,
                focusProgram = copy.focusProgram,
                focusProgress = copy.focusProgress,
                complete = copy.complete,
                dash = copy.dash,
                statusToday = status.statusToday,
                statusUpcoming = status.statusUpcoming,
                statusInProgress = status.statusInProgress,
                statusCompleted = status.statusCompleted,
                statusMissed = status.statusMissed,
                statusRest = status.statusRest,
            )
        }

        private suspend fun loadCore(language: String) = CoreStrings(
        ...
        private suspend fun loadCopy(language: String) = CopyStrings(
        ...
        private suspend fun loadStatus(language: String) = StatusStrings(
        ...
        private data class CoreStrings(
            val title: String,
            val activeProgram: String,
            val activePlan: String,
            val noActiveProgram: String,
        ...
        private data class CopyStrings(
            val subtitleActive: String,
            val subtitleCompleteToday: String,
            val subtitleRest: String,
            val subtitleProgramComplete: String,
