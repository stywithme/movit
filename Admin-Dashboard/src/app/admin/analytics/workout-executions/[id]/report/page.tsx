...
export default function WorkoutExecutionReportPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      setData(await analyticsService.workoutExecutionReport(params.id));
    } finally {
      setLoading(false);
    }
  }, [params.id]);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Workout Execution Report" description={`${data?.exercise?.name ?? ''} — ${data?.user?.name ?? ''}`} />
      ...
      <DataTable
        ...
        emptyDescription="This workout execution has no rep-level metrics."
      />
    </div>
  );
}
