  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <StatCard title="Changes Applied" value={formatNumber(data?.changes)} />
        <StatCard title="Active Rules" value={formatNumber(data?.activeRules)} />
        <StatCard title="Avg Success Streak" value={(data?.streaks.avgSuccess ?? 0).toFixed(1)} />
        <StatCard title="Avg Regression Streak" value={(data?.streaks.avgRegression ?? 0).toFixed(1)} />
      </div>

      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <ChartCard title="Progression Changes" loading={loading} empty={!data?.trend?.length}>
          <LineTrend data={data?.trend ?? []} />
        </ChartCard>
        <ChartCard title="Decision Types" loading={loading} empty={!data?.decisions?.length}>
