...
class TrainingFrameCaptureCoordinatorTest {
    ...
    fun jointDanger_requestsDangerCapture() = runBlocking {
        val manager = MovitPeakFrameCaptureManager()
        val coordinator = coordinator(manager)
        coordinator.onJointState("left_knee", JointState.DANGER, repInProgress = 2, phase = Phase.DOWN)
        assertEquals(MovitPeakCaptureType.DANGER_FRAME, manager.captures().single().captureType)
    }
