...
class TrainingActivity : AppCompatActivity(), PoseLandmarkerHelper.PoseDetectionListener {
    ...
    private enum class TrainingState {
        ...
        PAUSED,
        COMPLETED,
        VISIBILITY_PAUSED,      // Paused due to joints not visible
        VISIBILITY_SETUP_POSE   // After visibility returns, validating startPose
    }
    private var trainingState = TrainingState.SETUP_POSE
    
    // Saved state for visibility resume
    private var visibilitySavedRepCount: Int = 0
    ...
    // Guard against spillover reps during hot-swap
    // When true, rep counting is frozen until hot-swap completes
    @Volatile
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Get workout integration extras (for sequential mode from WorkoutActivity)
        maxRepsThisSession = intent.getIntExtra(EXTRA_MAX_REPS_THIS_SESSION, -1)
            .takeIf { it > 0 }
        sessionStartTime = System.currentTimeMillis()
        
        // Target overrides from WorkoutActivity (sequential mode)
        ...
        val targetDurationOverride = intent.getIntExtra(EXTRA_TARGET_DURATION_OVERRIDE, -1)
            .takeIf { it > 0 }?.let { it * 1000L } // Convert seconds to milliseconds
        
        if (maxRepsThisSession != null) {
            Log.d(TAG, "Workout mode: maxRepsThisSession = $maxRepsThisSession")
        }
        if (targetRepsOverride != null || targetDurationOverride != null) {
            Log.d(TAG, "Target overrides: reps=$targetRepsOverride, durationMs=$targetDurationOverride")
        }
        
        // Check for Workout Mode (Hot-Swap)
        ...
        if (isWorkoutMode && workoutName != null) {
            ...
            // Single Exercise Mode (with optional target overrides from WorkoutActivity sequential mode)
            loadExercise(exerciseName, difficultyStr, targetRepsOverride, targetDurationOverride)
        }
    ...
    /**
     * Load single exercise (standalone mode or sequential workout mode)
     * 
     * @param targetRepsOverride Override reps from WorkoutExercise.target (null = use exercise default)
     * @param targetDurationMsOverride Override duration from WorkoutExercise.target in ms (null = use default)
     */
    private fun loadExercise(
        ...
        // Initialize training engine with optional target overrides
    ...
    private fun updateUIForState(state: TrainingState) {
        ...
        when (state) {
            ...
            TrainingState.VISIBILITY_PAUSED -> {
                // Keep hero counter visible but show paused state
                binding.heroCounterContainer.visibility = View.VISIBLE
                binding.tvProgress.visibility = View.VISIBLE
                binding.progressContainer.visibility = View.VISIBLE
                
                // Show play icon (paused)
                updatePlayPauseIcon(isPlaying = false)
            }
            
            TrainingState.VISIBILITY_SETUP_POSE -> {
                // Show pose requirements like initial SETUP_POSE but keep rep counter visible
                binding.setupPosePanel.visibility = View.VISIBLE
                binding.countdownPanel.visibility = View.GONE
                binding.heroCounterContainer.visibility = View.VISIBLE  // Keep visible to show saved reps
                binding.tvProgress.visibility = View.VISIBLE
                binding.progressContainer.visibility = View.VISIBLE
                binding.completedPanel.visibility = View.GONE
                
                // Show play icon (waiting for pose)
                updatePlayPauseIcon(isPlaying = false)
            }
        }
    }
    ...
    private fun startTraining() {
        ...
        if (isWorkoutMode) {
            ...
            // Enable training mode in skeleton overlay (with front camera mirroring if needed)
        ...
        workoutObserverJob = lifecycleScope.launch {
            ...
            launch {
                engine.repCount.collect { count ->
                    ...
                    // Guard: Skip if already switching to prevent spillover reps
                    if (count > 0 && count > currentRepsInSession && !isSwitchingExercise) {
                        ...
                        if (count >= repsLimit && trainingState == TrainingState.TRAINING) {
                            // Set guard flag IMMEDIATELY to prevent spillover
    ...
    private fun handleWorkoutRepLimitReached(completedReps: Int) {
        ...
        // Get correct reps from current engine for accuracy tracking
    ...
    private fun performHotSwap(nextExerciseName: String, repsThisSession: Int) {
        ...
        // Reset session rep count and clear switching guard
        currentRepsInSession = 0
        ...
        // Update skeleton overlay with new tracked joints (with front camera mirroring if needed)
    ...
    private fun completeWorkout() {
        ...
        // Show workout summary with real accuracy
        ...
        binding.tvSummaryAccuracy.text = "${workoutEngine.getOverallAccuracy().toInt()}%"
     ...
     * 
     * In Workout Mode (Hot-Swap): Returns total reps/accuracy across all exercises
     * In Single Exercise Mode: Returns current engine's reps/accuracy
     */
    private fun finishWithResult() {
        val sessionDuration = System.currentTimeMillis() - sessionStartTime
        
        val resultIntent = android.content.Intent().apply {
            if (isWorkoutMode && workoutTrainingEngine != null) {
                // Workout Mode: Return aggregated totals with real accuracy
                val workoutEngine = workoutTrainingEngine!!
                val progressInfo = workoutEngine.getProgressInfo()
                putExtra(RESULT_REPS_COMPLETED, progressInfo.totalRepsCompleted)
                putExtra(RESULT_DURATION_MS, sessionDuration)
                putExtra(RESULT_ACCURACY, workoutEngine.getOverallAccuracy())
                putExtra(RESULT_IS_COMPLETED, workoutEngine.isWorkoutCompleted.value)
            } else {
                // Single Exercise Mode
                val engine = trainingEngine
    ...
    private fun handleFeedbackEvent(event: FeedbackEvent) {
        when (event) {
            ...
            is FeedbackEvent.VisibilityWarning -> {
                // Show warning message with remaining time
                Log.d(TAG, "Visibility warning: ${event.remainingBeforePauseMs}ms before pause")
                val message = event.message.en
                binding.glassmorphicMessage.showMessage(
                    message,
                    GlassmorphicMessageView.TYPE_WARNING,
                    durationMs = 1000  // Short duration as it updates frequently
                )
                // Trigger warning vignette
                binding.vignetteOverlay.showWarning()
            }
            
            is FeedbackEvent.VisibilityPaused -> {
                // Training paused due to visibility - show overlay and save state
                Log.d(TAG, "Visibility paused at rep ${event.savedRepCount}")
                handleVisibilityPaused(event)
            }
            
            is FeedbackEvent.VisibilityResumeCountdown -> {
                // Joints visible again - start startPose validation (NOT countdown directly)
                Log.d(TAG, "Visibility resume - starting startPose validation from rep ${event.resumeFromRep}")
                handleVisibilityResumeSetupPose(event)
            }
            
            is FeedbackEvent.VisibilityResumed -> {
                // Resume complete - back to training
                Log.d(TAG, "Visibility resumed at rep ${event.repCount}")
    ...
    /**
     * Handle visibility paused event - training paused due to joints not visible
     */
    private fun handleVisibilityPaused(event: FeedbackEvent.VisibilityPaused) {
        // Update state
        trainingState = TrainingState.VISIBILITY_PAUSED
        
        // Save rep count for later resume
        visibilitySavedRepCount = event.savedRepCount
        
        // Show persistent message
        binding.glassmorphicMessage.showMessage(
            event.message.en,
            GlassmorphicMessageView.TYPE_ERROR,
            durationMs = -1  // Persistent until dismissed
        )
        
        // Show error vignette
        binding.vignetteOverlay.showError()
        
        // Update UI to show paused state with saved rep count
        binding.tvRepCount.text = event.savedRepCount.toString()
        
        // Audio feedback
        if (!isVideoMode) {
            feedbackManager?.speak("Training paused. Step back into frame.")
        }
        
        Log.d(TAG, "Visibility paused - saved rep: ${event.savedRepCount}, phase: ${event.savedPhase}")
    }
    
    /**
     * Handle visibility resume - joints visible again, start startPose validation
     * User must be in correct startPose before countdown starts
     */
    private fun handleVisibilityResumeSetupPose(event: FeedbackEvent.VisibilityResumeCountdown) {
        // Save rep count
        visibilitySavedRepCount = event.resumeFromRep
        
        // Transition to VISIBILITY_SETUP_POSE state
        trainingState = TrainingState.VISIBILITY_SETUP_POSE
        
        // Reset pose validation frame counter
        poseValidFrames = 0
        ...
        // Clear error vignette
        binding.vignetteOverlay.clear()
        
        // Show pose requirements (same as initial SETUP_POSE)
        showPoseRequirements()
        binding.tvPoseStatus.visibility = View.VISIBLE
        
        Log.d(TAG, "Visibility resume - waiting for startPose validation, saved rep: ${event.resumeFromRep}")
    }
    
    /**
     * Start visibility resume countdown after startPose is validated
     */
    private fun startVisibilityResumeCountdown() {
        ...
        countdownTimer = object : CountDownTimer((COUNTDOWN_SECONDS * 1000).toLong(), 1000) {
            ...
            override fun onFinish() {
                // Animate "GO!"
                AnimationUtils.animateGoText(binding.tvCountdown) {
                    // Resume training from visibility pause
                    resumeFromVisibilityPause(visibilitySavedRepCount)
                }
                feedbackManager?.speakGo()
            }
        }.start()
    ...
    /**
     * Resume training after visibility countdown completes
     */
    private fun resumeFromVisibilityPause(repCount: Int) {
        // Hide countdown UI
        ...
        // Tell training engine to resume
        trainingEngine?.resumeFromVisibilityPause()
        
        // Update rep count display (should be preserved)
        binding.tvRepCount.text = repCount.toString()
        
        Log.d(TAG, "Resumed from visibility pause at rep $repCount")
    }
            ...
            when (trainingState) {
                ...
                TrainingState.TRAINING -> {
                    if (isWorkoutMode) {
                        ...
                        val engine = trainingEngine
                        if (engine != null) {
                            engine.processFrame(angles, smoothedLandmarks, result.isFrontCamera)
                        } else {
                            Log.e(TAG, "TrainingEngine is null in TRAINING state!")
                        }
                    } else {
                        trainingEngine?.processFrame(angles, smoothedLandmarks, result.isFrontCamera)
                    }
                }
                
                TrainingState.VISIBILITY_PAUSED -> {
                    // Don't process frames while visibility paused
                    // Just update skeleton overlay for visual feedback
                }
                
                TrainingState.VISIBILITY_SETUP_POSE -> {
                    // Validate startPose after visibility returns (same as initial SETUP_POSE)
                    val isValid = validateStartPose()
        ...
        lifecycleScope.launch(Dispatchers.Default) {
            try {
                ...
                if (poseResult != null) {
                    ...
                    // Apply front camera correction: swap LEFT/RIGHT angles
                    // Note: Video mode doesn't use front camera mirroring, so no swap needed
                    val angles = if (poseResult.isFrontCamera) rawAngles.mirrored() else rawAngles
                    
                    // Process training logic on background thread
                    trainingEngine?.processFrame(angles, smoothedLandmarks, poseResult.isFrontCamera)
