...
/**
 * Hold FSM, per-exercise form counters, and [HoldStatus] mirroring. Extracted from
...
class HoldExerciseCoordinator(
    ...
    fun installCallbacks(
        ...
        setExerciseCompleted: (Boolean) -> Unit
    ) {
        ...
        timer.onCompleted = { totalMs, gracePeriodsUsed ->
            ...
            exerciseFormQuality = formQuality
            exerciseErrorCount = finalResult?.getTotalErrorCount() ?: 0
            exerciseJointErrorMap = emptyMap()
            refreshStatus(publishStatus)
