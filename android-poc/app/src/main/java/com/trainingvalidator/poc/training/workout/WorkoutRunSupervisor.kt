...
/**
 * WorkoutRunSupervisor - Unified State Machine for a single-exercise workout run
 * 
...
class WorkoutRunSupervisor {
    ...
    companion object {
        private const val TAG = "WorkoutRunSupervisor"
    ...
    fun onExerciseLoaded() {
        if (_state.value == WorkoutRunState.IDLE) {
            if (isVideoMode) {
     ...
     */
    fun reset() {
        _state.value = WorkoutRunState.IDLE
        pauseReason = null
        noPoseStartTime = 0L
     ...
     */
    private fun handleGlobalSignal(signal: SupervisorSignal, currentState: WorkoutRunState): Boolean {
        when (signal) {
            is SupervisorSignal.StopRequested -> {
                if (currentState != WorkoutRunState.IDLE && currentState != WorkoutRunState.COMPLETED) {
            ...
            is SupervisorSignal.ActivityPaused -> {
                if (currentState == WorkoutRunState.TRAINING && !activityPausedEngine) {
    ...
    private fun handleSetupPose(signal: SupervisorSignal) {
        when (signal) {
            ...
            is SupervisorSignal.PoseConfirmed -> {
                ...
                countdownInvalidStartMs = 0L
                countdownFrozen = false
                transitionTo(WorkoutRunState.COUNTDOWN)
            ...
            is SupervisorSignal.PoseFrame -> {
                if (pauseReason == PauseReason.NO_POSE) {
                    if (isVideoMode) {
                        ...
                        transitionTo(WorkoutRunState.RESUME_SETUP)
                        emit(SupervisorAction.ShowSetupPose)
    ...
    private fun handleResumeSetup(signal: SupervisorSignal) {
        when (signal) {
            ...
            is SupervisorSignal.PoseConfirmed -> {
                transitionTo(WorkoutRunState.RESUME_COUNTDOWN)
                emit(SupervisorAction.StartCountdown)
    ...
    private fun handleResumeCountdown(signal: SupervisorSignal) {
        ...
        when (signal) {
            ...
            is SupervisorSignal.PoseInvalid -> {
                handleCountdownPoseLost(cfg)
                if (_state.value == WorkoutRunState.SETUP_POSE) {
                    transitionTo(WorkoutRunState.AUTO_PAUSED)
    ...
    private fun transitionTo(newState: WorkoutRunState) {
