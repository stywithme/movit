...
import com.trainingvalidator.poc.training.models.OutwardDirection
...
/**
 * Single source of truth: per-joint quality [JointEval] for a frame.
 */
class JointEvaluator(
    ...
    /**
     * @param currentAngles Smoothed angles (authoritative for joint quality)
     * @param currentPhase Current exercise phase (secondary phaseRanges use this)
     */
    fun evaluate(
        ...
        for (joint in trackedJoints) {
            val angle = currentAngles[joint.joint]
            if (angle == null) {
    ...
    private fun evaluateOne(
        ...
        val upStateRanges: StateRanges? = if (joint.hasStateUpDownRanges()) {
            joint.getStateUpRange()
        } else {
            stateRanges ?: if (joint.hasStateHoldRange()) joint.getStateHoldRange() else null
        }
        val downStateRanges: StateRanges? = if (joint.hasStateUpDownRanges()) {
        ...
        val state: JointState = applyHysteresis(
            joint.joint, rawState, angle, stateRanges, isOutwardFallback
        )

        val phaseNameForMessages: String? =
            if (joint.role == JointRole.SECONDARY && joint.phaseRanges != null) {
                mapPhaseToName(currentPhase)
            } else {
                null
            }
        val messages = joint.getMessagesForState(state, zoneType, phaseNameForMessages)
    ...
    private fun applyHysteresis(
        ...
        if (previousState == null) {
            val initial = if (rawState == JointState.DANGER && !isOutwardFallback) {
                JointState.WARNING
            } else {
                rawState
            }
        ...
        val hysteresisDegree: Double = policy.statePairHysteresis(previousState, rawState)
        ...
        if (stateRanges != null && hysteresisDegree > 0) {
            val ok = checkTransitionConfirmed(
