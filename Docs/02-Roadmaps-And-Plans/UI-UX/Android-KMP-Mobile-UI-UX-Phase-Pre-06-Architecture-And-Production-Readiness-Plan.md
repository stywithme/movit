...
آخر تحديث: 2026-06-09 (بدء التنفيذ — WS-A + WS-G + WS-B جزئي)
...
| **WS-B** Training engine | 🔄 جزئي | موديول `:core:training-engine` — `OneEuroFilter` + `JointAngleCalculator` + 7 tests |
| **WS-C** Launcher gate | ⏳ | لم يبدأ |
| **WS-D** Secure storage | ⏳ | لم يبدأ |
| **WS-E** UI scorecards | ⏳ | لم يبدأ |
| **WS-F** Visual smoke | ⏳ | لم يبدأ |
...
:core:training-engine:testDebugUnitTest       ✅ (7)
...
- [ ] access/refresh tokens لا تُحفظ في storage عادي في المسار الإنتاجي.
- [ ] لكل صفحة منجزة scorecard يشرح نسبة الإنجاز.
- [ ] Home/Train/Auth/Profile على الأقل لديهم visual QA checklist أو screenshot smoke.
- [ ] قرار launcher مكتوب: متى يتحول الإنتاج من legacy إلى Movit shell.
