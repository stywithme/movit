...
## 16) تشخيص تأخّر الـ Skeleton عن الحركة السريعة (تتبع كود — 2026-06-12)
...
1. **R2 إصلاح الإحداثيات الثلاثة** (mirror أمامية + rotation عبر `ImageProcessingOptions`/OutputImageRotation + تعويض aspect/crop في الرسم بمعرفة أبعاد الإطار) — يصلح «الالتصاق المكاني» فوراً وهو bug مستقل عن الزمن.
2. **R4 المحرك خارج Main** (Default dispatcher لمسار ProcessFrame).
3. **R1 فصل مسار الرسم عن مسار المحرك**: قناة «أحدث نقاط» من callback الـ detector إلى الـ overlay مباشرة (state داخلي للـ Canvas فقط/View مخصص) — بلا UiState وبلا انتظار المحرك وبلا إسقاط.
4. **R3 قتل `toBitmap`**: `setOutputImageFormat(RGBA_8888)` + builder مباشر أو bitmap pool (نمط legacy) + تمرير الدوران للموديل بدل تدوير البكسلات.
5. **R5 تنعيم مزدوج المستوى**: الرسم خام أو OneEuro بـ beta عالٍ جداً؛ المحرك كما هو؛ وتوصيل التفضيلات فعلياً.
