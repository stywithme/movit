...
> **التاريخ:** 2026-06-13  
> **النطاق:** `kmp-app/app` فقط (بدون `core/data` أو منطق المزامنة).
...
### نتائج البناء والاختبار (Gradle من `kmp-app`)
...
### نتائج البناء (Gradle من `kmp-app`)
...
توثيق: `kmp-app/scripts/README-cold-offline-bundle.md`
...
**التاريخ:** 2026-06-13  
**النطاق:** `kmp-app/` — تدقيق جاهزية قشرة KMP قبل قلب اللانشر الإنتاجي  
...
|--------|-------|----------------|
| A1 | `kmp-app/gradle.properties` أو `app/build.gradle.kts` | `movit.shell.launcher.enabled` default → `true` |
...
**التاريخ:** 2026-06-13  
**الأمر:** `.\gradlew testDebugUnitTest --continue --console=plain` من `kmp-app/`  
...
**دليل git:** `git diff HEAD -- kmp-app/app/src/test/` كان فارغاً على كل الملفات عدا ما أُصلح لاحقاً؛ `PoseApp.kt` يظهر diff هجرة (`BackgroundSyncScheduler`, `MovitAndroidRuntime`).
...
|--------|-------|--------|---------|
| **A1** | `kmp-app/gradle.properties` | 17–19 | `movit.shell.launcher.enabled=true` (مع تعليق rollback) |
| **A1** | `kmp-app/app/build.gradle.kts` | 9–10 | fallback `?: true` عند غياب الخاصية |
| **A2** | `kmp-app/gradle.properties` | 22 | `movit.training.kmp.enabled=true` (كان مفعّلاً مسبقاً) |
| **A2** | `kmp-app/app/build.gradle.kts` | 12–13 | fallback `?: true` |
...
### نتائج البناء (من `kmp-app`، PowerShell)
