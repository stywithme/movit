...
الرفض الصريح للـ AI Generator، Periodization Engine المعقد، Planned Workout Block entity، Rules Engine، وFailure-based prompts — كلها قرارات ممتازة ومتسقة مع ACSM 2026.
...
adaptationFocus + role + intent + progressionMode + effectiveProgressionMode — هذا كثير جداً على PlannedWorkoutItem.
...
الحقول المطلوبة على PlannedWorkoutItem (بدلاً من 5 حقول):
...
تبسيط PlannedWorkoutItem إلى 3 حقول رئيسية فقط: role, intent, progressionOverride (Json).
...
إضافة حقل coachingNotes Json على Program و PlannedWorkoutItem — يسمح للمدرب بكتابة توجيهات نصية حرة (هذا أهم من كثير من الحقول النظرية).
اقتراح هيكل نهائي مبسط لـ PlannedWorkoutItem:
...
الخلاصة النهائية
الـ Blueprint ممتاز في الرؤية والالتزام بالمبادئ، لكنه مبالغ في التعقيد في طبقة PlannedWorkoutItem و Exercise Family.
