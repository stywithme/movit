# Session Details & Customization (Day View) Redesign Plan

## 1. Primary Goal
Transform the "Day View" (`ProgramSessionActivity`) into a highly interactive, premium training dashboard. The user should be able to clearly see their sessions, understand the exercises at a glance (with images and specific metrics), and easily customize the workout (edit, swap, reorder) through a smart, context-aware interface.
...
### B. Session Cards (`item_day_session_card.xml`)
- When collapsed: Looks like the clean cards we designed earlier.
- When expanded: Reveals a beautiful vertical timeline.
- **Edit Mode Bar**: Should feel integrated, not like an afterthought. Clear icons for Rename, Delete, and Reorder (Up/Down) the entire session.

### C. Timeline Items (`item_session_timeline_item.xml`)
...
### D. Smart Edit Dialog (`dialog_session_item_edit.xml`)
...
1. **Update `dialog_session_item_edit.xml`**: Restructure into a clean, grid-like form using `TextInputLayout`.
2. **Update `item_session_timeline_item.xml`**: Make the exercise thumbnail larger, improve typography.
3. **Refactor `ProgramSessionActivity.kt` (Edit Logic)**:
...
4. **Update `item_day_session_card.xml`**: Refine the padding and shadows to match the premium aesthetic.